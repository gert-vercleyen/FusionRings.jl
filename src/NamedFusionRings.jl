module NamedFusionRings

using ..Types: FusionRing, fusion_tensor
...
export ty_fusion_ring, near_group_fusion_ring

# ------------------ Predefined / Named Fusion Rings ------------------
...
function ty_fusion_ring(G::Vector{String})
    ...
    for i in 1:n, j in 1:n
        k = ((i-1) + (j-1)) % n + 1; N[i, j, k] = 1
    end
    m = r
    for i in 1:n; N[i, m, m] = 1; N[m, i, m] = 1; end
    for i in 1:n; N[m, m, i] = 1; end
    fusion_ring(N; labels=labs, name="TY(" * join(G,",") * ")")
end

function near_group_fusion_ring(G::Vector{String}; n::Int=1)
    n ≥ 0 || error("n must be ≥ 0")
    nG = length(G); labs = vcat(G, ["X"])
    r = nG+1; N = fill(0, r,r,r); X = r
    for i in 1:nG, j in 1:nG
        k = ((i-1) + (j-1)) % nG + 1; N[i, j, k] = 1
    end
    for i in 1:nG; N[i, X, X] = 1; N[X, i, X] = 1; end
    for i in 1:nG; N[X, X, i] = 1; end
    if n>0; N[X, X, X] = n; end
