...
"""perm_vec_qd(r; order = :increasing) – permutation that sorts  non‑vacuum
...
vector σ such that `permute(r1, sigma)` is identical to `r2`, or `nothing` if no
such sigma exists.  For ranks \leq `exhaustive_limit`  search is exhaustive;
otherwise  heuristic backtracking search is used."""
function which_permutation(r1::FusionRing, r2::FusionRing; exhaustive_limit::Int = 8)
    ...
    # 2. Greedy match remaining particles by  fusion with vacuum and FP‑dim
    for i in 2:n
        if perm[i] == 0
...
"""register_known_ring!(r) – store `r` in  in‑memory catalogue so later
    rings can be matched quickly  `replace_by_known`."""
